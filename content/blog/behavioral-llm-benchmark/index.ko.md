...
당연히 없다. 우리는 지원자가 *어떻게* 문제를 푸는지 본다. 모호함을 어떻게 다루는지, 첫 번째 접근이 실패했을 때 전략을 바꾸는지. 지식(knowledge)이 아니라 행동(behavior)을 평가하는 것이다.

그런데 LLM 평가는 정확히 이걸 하고 있다. 객관식, 빈칸 채우기, 스펙 보고 함수 짜기 — 그걸 "평가"라고 부른다. MMLU 점수와 HumanEval 통과율로 모델을 줄 세우면서, 그 숫자가 모든 걸 말해준다고 믿는다.
