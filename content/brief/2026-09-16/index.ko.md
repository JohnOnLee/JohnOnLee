...
인디 개발자에게 이 릴리스가 유용한 이유는 모델 이름보다 코드와 원가 구조에 있다. ASR, LLM, TTS를 각각 붙이고 그 사이 턴 관리와 끊김 처리를 하던 코드가 세션 하나로 줄어듭니다. 지연 예산을 세 조각으로 나눠 쓰던 구조도 단순해집니다.

도구가 도는 동안 모델이 계속 말할 수 있다는 점은 대기 화면 설계를 바꿉니다. 기존 캐스케이드에서 쓰던 "확인 중입니다" 클립을 빼고, 도구 호출이 끝날 때까지 모델이 진행 상황을 말하게 두면 사용자가 느끼는 대기 시간이 달라지는지 바로 비교할 수 있다.

분당 과금에서는 통화 길이가 곧 원가죠. 10분 통화 $0.23, 60분 $1.38은 어림잡아 선형에 가깝습니다. 무료 통화를 붙이는 제품이라면 통화 시간 상한과 유휴 세션 종료가 먼저다. 지금 쓰는 STT, LLM, TTS 단가를 합산해 같은 10분 통화 비용과 나란히 놓으면 스택 교체 시점도 숫자로 잡을 수 있습니다.

모델 교체 여지는 추상화 레이어에 남겨둘 필요가 있다. Live API를 직접 부르면 구글에 묶이니, LiveKit이나 Pipecat을 사이에 두고 세션 코드는 유지한 채 모델만 갈아끼우는 구조가 낫습니다. 실시간 시각 입력까지 붙이면 사진 속 청구서 번호나 오류 화면을 읽는 지원 흐름도 한 세션 안에서 시험할 수 있습니다.

## 먼저 검증할 것들

τ-Voice-banking 35.1%는 낮습니다. 은행 업무 같은 규제 영역의 에이전트 완주율이 이 수준이면 완전 자동화를 약속하는 제품 문구는 아직 이르죠.

무료 티어도 조심할 대목이다. 구글은 무료 티어 트래픽을 "Used to improve our products"로 표시하고, 유료 등급만 이 조항에서 빠집니다. 실제 고객 음성을 받는 제품이라면 유료 등급을 기본값으로 보는 편이 안전합니다.

장시간 통화 품질은 아직 공개 숫자만으로 판단하기 어렵다. 공개된 숫자는 벤치마크와 짧은 시연 중심이니 30분 이상 통화에서의 맥락 유지와 끼어들기 처리는 직접 재봐야 합니다.

비용은 대화 길이를 따라간다. 세션을 열어 두는 구조라면 유휴 시간도 과금 대상입니다. 상한 없는 통화를 허용하는 요금제는 손실을 키울 뿐입니다.
